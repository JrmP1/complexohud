...
function updateDisplayHud()
	...
	if IsPedInAnyVehicle(ped) then
		...
		SendNUIMessage({ vehicle = true, talking = talking, health = GetEntityHealth(ped) - 100, armour = armour, street = streetName, direction = flexDirection, radio = radioDisplay, voice = voice, oxigen = clientOxigen, suit = divingMask, fuel = fuel, speed = speed, showbelt = showBelt, seatbelt = beltLock })
	else
		if IsMinimapRendering() then
			DisplayRadar(false)
		end

		SendNUIMessage({ vehicle = false, talking = talking, health = GetEntityHealth(ped) - 100, armour = armour, street = streetName, direction = flexDirection, radio = radioDisplay, voice = voice, oxigen = clientOxigen, suit = divingMask })
	end
end
-- lzlasyOOAj4f9bOT
-----------------------------------------------------------------------------------------------------------------------------------------
